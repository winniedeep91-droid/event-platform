  ...
  return (
    <Stack>
      ...
      <Alert tone="info" title="WooCommerce coupons">
        Coupons are created and validated in WooCommerce. EventOS links them to marketing campaigns
        for tracking. Discount campaigns created in EventOS automatically generate WooCommerce
        coupons.
      </Alert>
      ...
      <Card title={`Coupons${total > 0 ? ` (${total.toLocaleString()})` : ""}`}>
        <Stack>
          ...
          {isLoading ? (
            ...
            <Alert tone="danger" title="Could not load coupons">
              ...
              <DataTable
                ...
                emptyDescription={
                  search || status
                    ...
                    : "Coupons will appear here after the first sync."
                }
              />
              {totalPages > 1 && (
                <Pagination
      ...
      {assignTarget && <AssignModal coupon={assignTarget} onClose={() => setAssignTarget(null)} />}
    </Stack>
  );
}
