...
function Dashboard() {
  ...
  if (error)
    return <div className="eventos-notice eventos-notice--error">{(error as Error).message}</div>;
  if (!data) return null;
  ...
  return (
    <>
      <Header
        title={String(data.general.business_name || "EventOS")}
        subtitle="Core configuration overview"
      />
      <div className="eventos-grid">
        <section className="eventos-card">
          <h2 className="eventos-card__title">System status</h2>
          <p
            className={`eventos-card__metric eventos-status ${system.healthy ? "eventos-status--ok" : "eventos-status--fail"}`}
          >
          ...
          <ul className="eventos-list">
            {system.checks.map((check) => (
              <li key={check.id}>
                ...
                <span
                  className={`eventos-status ${check.passing ? "eventos-status--ok" : "eventos-status--fail"}`}
                >
                  {check.value}
                </span>
              </li>
        ...
        <section className="eventos-card">
          ...
          <ul className="eventos-list">
            <li>
              ...
              <span>{system.plugin_version}</span>
            </li>
            <li>
              <span>Database schema</span>
              <span>{system.db_version}</span>
            </li>
            <li>
              <span>WordPress</span>
              <span>{system.wordpress_version}</span>
            </li>
            <li>
              <span>PHP</span>
            ...
            <li>
              <span>MySQL</span>
              <span>{system.mysql_version}</span>
            </li>
            <li>
              <span>WooCommerce</span>
              <span>
                {system.woocommerce.active
                  ? system.woocommerce.version || "Active"
                  : "Not installed"}
              </span>
            </li>
          </ul>
        ...
        <section className="eventos-card">
          ...
          <ul className="eventos-list">
            <li>
              <span>Files</span>
              <span>{system.storage.file_count}</span>
            </li>
            <li>
              <span>Media attachments</span>
              <span>{system.storage.attachments}</span>
            </li>
          </ul>
        </section>
        ...
        <section className="eventos-card">
          ...
          <ul className="eventos-list">
            ...
            <li>
              ...
              <span>
                {user.eventos_roles.length ? user.eventos_roles.join(", ") : "None assigned"}
              </span>
            </li>
          </ul>
        </section>
        ...
        <section className="eventos-card">
          ...
          <ul className="eventos-list">
            {menu
              .filter((item) => item.view !== "dashboard")
              .map((item) => (
                <li key={item.slug}>
        ...
        <section className="eventos-card">
          ...
          {upcoming.length ? (
            ...
            <p className="eventos-empty">
              No events yet. The Events module will populate this card.
            </p>
          )}
        </section>
          ...
          {activity.length ? (
            <ul className="eventos-list">
              {activity.map((entry) => (
                <li key={entry.id}>
                  <span>
                    {entry.action.replace(/_/g, " ")} — {entry.user.name}
                  </span>
...
function SettingsView({ group }: { group: string }) {
  ...
  if (error)
    return <div className="eventos-notice eventos-notice--error">{(error as Error).message}</div>;
  if (!definition)
    return <div className="eventos-notice eventos-notice--error">Unknown settings group.</div>;

  return (
    <>
      <Header title={definition.label} subtitle={definition.description} />
      {mutation.error ? (
        <div className="eventos-notice eventos-notice--error">
...
function TeamView() {
  ...
  return (
    <>
      <Header
        title="Team"
        subtitle="Assign EventOS roles to WordPress users and invite new team members."
      />
